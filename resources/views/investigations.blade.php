				...
				<div class="table-responsive table mt-2">
					<table class="table my-0 text-center align-middle">
						...
						@foreach ( $investigations as $investigation )
							<tr>
								<td><a class="nav-link" href="/investigation/{{ $investigation->id }}">{{ $investigation->id }}</a></td>
								<td>{{ ucfirst( __( 'investigation-topic-' . $investigation->type ) ) }}</td>
								<td><a class="nav-link" href="/user/{{ $investigation->subject->id }}">{{ $investigation->subject->username }}</a>
								</td>

										...
										@foreach ( json_decode( $appeal->review, true ) as $name => $data )
											...
											@else
												@php
													...
													$color = ( $value === 1 ) ? 'success' : ( ( $value === -1 ) ? 'danger' : 'secondary' );
												@endphp
												<div class="row mb-2 mt-2">
													<div class="col-10 fw-bold">{{ __( 'appeal-review-' . $appeal->type . '-' . $name ) }}</div>
													<div class="col-2"><p class="btn btn-outline-{{ $color }} fw-bold disabled">{{ __( $longOpt ) }}</p></div>
												</div>
											@endif
										@endforeach
									</div>
						...
						@unless ( $appeal->reviewed )
							<div class="row">
								<div class="col" style="text-align: center;">
									<button class="btn btn-primary text-white" type="button" data-bs-target="#modal-1" data-bs-toggle="modal">
										<i class="fa-solid fa-eye"></i> {{ __( 'appeal-edit-review' ) }}
									</button>
								</div>
							</div>
						@endunless
										...
										<div class="modal-body text-center">
											@foreach ( config( 'app.appeals.' . $appeal->type ) as $label => $outcomes )
												...
												@foreach ( $outcomes as $opt => $value )
													...
													<label class="btn btn-outline-{{ $color }}" for="{{ $label . '-' . $sc }}">{{ __( $opt ) }}</label>
												@endforeach
											@endforeach
											<hr>
